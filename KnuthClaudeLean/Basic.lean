...
theorem claudeDir_cycle0_s0_j {m : ℕ} [NeZero m] (hm : 1 < m) (v : Vertex m)
    (hs : fiber v = 0) (hj : v.2.1 = -1) :
    claudeDir v 0 = 0 := by
  simp [claudeDir, hs, hj]

/-- Cycle 0 bumps k when s = 0 and j ≠ −1. -/
theorem claudeDir_cycle0_s0_nj {m : ℕ} [NeZero m] (hm : 1 < m) (v : Vertex m)
    (hs : fiber v = 0) (hj : v.2.1 ≠ -1) :
    claudeDir v 0 = 2 := by
  simp [claudeDir, hs, hj]

/-- Cycle 0 bumps j when 0 < s < m−1 and i ≠ −1. -/
theorem claudeDir_cycle0_mid_ni {m : ℕ} [NeZero m] (hm : 1 < m) (v : Vertex m)
    (hs0 : fiber v ≠ 0) (hs1 : fiber v ≠ -1) (hi : v.1 ≠ -1) :
    claudeDir v 0 = 1 := by
  simp [claudeDir, hs0, hs1, hi]

/-- Cycle 0 bumps k when 0 < s < m−1 and i = −1. -/
theorem claudeDir_cycle0_mid_i {m : ℕ} [NeZero m] (hm : 1 < m) (v : Vertex m)
    (hs0 : fiber v ≠ 0) (hs1 : fiber v ≠ -1) (hi : v.1 = -1) :
    claudeDir v 0 = 2 := by
  simp [claudeDir, hs0, hs1, hi]
...
theorem claudeDir_cycle1_mid {m : ℕ} [NeZero m] (hm : 1 < m) (v : Vertex m)
...
theorem claudeDir_cycle2_s0_nj {m : ℕ} [NeZero m] (hm : 1 < m) (v : Vertex m)
    (hs : fiber v = 0) (hj : v.2.1 ≠ -1) :
    claudeDir v 2 = 0 := by
  simp [claudeDir, hs, hj]

/-- Cycle 2 bumps k when s = 0 and j = −1. -/
theorem claudeDir_cycle2_s0_j {m : ℕ} [NeZero m] (hm : 1 < m) (v : Vertex m)
    (hs : fiber v = 0) (hj : v.2.1 = -1) :
    claudeDir v 2 = 2 := by
  simp [claudeDir, hs, hj]

/-- Cycle 2 bumps k when 0 < s < m−1 and i ≠ −1. -/
theorem claudeDir_cycle2_mid_ni {m : ℕ} [NeZero m] (hm : 1 < m) (v : Vertex m)
    (hs0 : fiber v ≠ 0) (hs1 : fiber v ≠ -1) (hi : v.1 ≠ -1) :
    claudeDir v 2 = 2 := by
  simp [claudeDir, hs0, hs1, hi]

/-- Cycle 2 bumps j when 0 < s < m−1 and i = −1. -/
theorem claudeDir_cycle2_mid_i {m : ℕ} [NeZero m] (hm : 1 < m) (v : Vertex m)
    (hs0 : fiber v ≠ 0) (hs1 : fiber v ≠ -1) (hi : v.1 = -1) :
    claudeDir v 2 = 1 := by
  simp [claudeDir, hs0, hs1, hi]
...
theorem claudeStep_fst_of_dir_ne_zero {m : ℕ} [NeZero m] (c : Fin 3) (v : Vertex m)
  ...
  rcases this with h1 | h1 <;> simp [claudeStep, h1, bumpAt]
...
theorem claudeStep_fst_cycle0 {m : ℕ} [NeZero m] (hm : 1 < m) (v : Vertex m)
...
theorem claudeStep_bijective {m : ℕ} [NeZero m] (hm : Odd m) (hm' : 1 < m) (c : Fin 3) :
  ...
  · by_cases hs1 : fiber v = -1
    · simp only [hs0, hs1, ite_false, ite_true]
...
private theorem returnMap0_generic {m : ℕ} [NeZero m] (hm : Odd m) (hm' : 1 < m)
  ...
  simp only [claudeStep, hdir3, bumpAt]
  refine Prod.ext rfl (Prod.ext ?_ ?_) <;> rw [zmod_natCast_m_sub_two (by omega)] <;> ring

/-- Return map for cycle 0, i = 0 case: j ≠ −1.
After m steps, (0, j, −j) ↦ (0, j−2, −(j−2)). -/
private theorem returnMap0_i0 {m : ℕ} [NeZero m] (hm : Odd m) (hm' : 1 < m)
    (j : ZMod m) (hj : j ≠ -1) :
...
private theorem returnMap0_im1 {m : ℕ} [NeZero m] (hm : Odd m) (hm' : 1 < m)
    (j : ZMod m) (hj : j ≠ -1) :
...
private theorem returnMap0_transition {m : ℕ} [NeZero m] (hm : Odd m) (hm' : 1 < m)
...
private theorem returnMap0_transition_to_last {m : ℕ} [NeZero m] (hm : Odd m) (hm' : 1 < m) :
...
private theorem returnMap0_wrap {m : ℕ} [NeZero m] (hm : Odd m) (hm' : 1 < m) :
...
private theorem returnMap1_ne2 {m : ℕ} [NeZero m] (hm : Odd m) (hm' : 1 < m)
    (i j : ZMod m) (hi : i ≠ 2) :
  ...
  simp only [claudeStep, hdir3, bumpAt]
  refine Prod.ext ?_ (Prod.ext rfl ?_) <;> rw [zmod_natCast_m_sub_two (by omega)] <;> ring

/-- Return map for cycle 1, i = 2 case.
After m steps, (2, j, −2−j) ↦ (0, j+2, −2−j). -/
private theorem returnMap1_eq2 {m : ℕ} [NeZero m] (hm : Odd m) (hm' : 1 < m)
    (j : ZMod m) :
  ...
  simp only [claudeStep, hdir3, bumpAt]
  refine Prod.ext ?_ (Prod.ext ?_ rfl) <;> rw [zmod_natCast_m_sub_two (by omega)] <;> ring
...
private theorem returnMap2_generic {m : ℕ} [NeZero m] (hm : Odd m) (hm' : 1 < m)
...
private theorem returnMap2_wrap {m : ℕ} [NeZero m] (hm : Odd m) (hm' : 1 < m)
    (j : ZMod m) (hj : j ≠ -1) :
  ...
  have hdir3 : claudeDir ((-1 : ZMod m), j + ((m - 2 : ℕ) : ZMod m), 2 - j) 2 = 0 :=
    claudeDir_cycle2_s1 hm' _ hfib_last
  simp only [claudeStep, hdir3, bumpAt]
  refine Prod.ext ?_ (Prod.ext ?_ ?_) <;> rw [zmod_natCast_m_sub_two (by omega)] <;> ring

/-- Return map for cycle 2, transition case: j = -1, i ≠ -1.
After m steps, (i, −1, 1−i) ↦ (i+1, −1, −i). -/
private theorem returnMap2_transition {m : ℕ} [NeZero m] (hm : Odd m) (hm' : 1 < m)
    (i : ZMod m) (hi : i ≠ -1) :
...
After m steps, (−1, −1, 2) ↦ (0, −3, 3). -/
private theorem returnMap2_transition_wrap {m : ℕ} [NeZero m] (hm : Odd m) (hm' : 1 < m) :
...
private theorem returnMap2_jblock {m : ℕ} [NeZero m] (hm : Odd m) (hm' : 1 < m)
  ...
  · subst hj
    simp only [neg_neg, show (-1 : ZMod m) - 2 = -3 from by ring,
               show -((-1 : ZMod m) - 2) = 3 from by ring]
...
Aubert and Schneider (1982) proved that no decomposition into three Hamiltonian 8-cycles
exists for the cube digraph with m = 2. -/

/-- No decomposition of the cube digraph on (ZMod 2)³ into three directed Hamiltonian 8-cycles
exists. -/
theorem no_decomposition_two :
    ¬ ∃ (σ : Fin 3 → Equiv.Perm (Vertex 2)),
      (∀ c, IsDirectedHamiltonianCycle (cubeDigraph 2) (σ c)) ∧
      (∀ v : Vertex 2, ∀ b : Fin 3,
        ∃! c : Fin 3, bumpAt b v = σ c v) := by
  sorry

/-! ## Generalizability

Given a Hamiltonian cycle for m = 3, we can attempt to "lift" it to general odd m ≥ 3.
The reduction map sends 0 ↦ 0, m−1 ↦ 2, and everything else ↦ 1. -/

/-- The reduction map from ZMod m to ZMod 3, sending 0 ↦ 0, −1 ↦ 2,
and everything else ↦ 1. -/
def reduce {m : ℕ} [NeZero m] (x : ZMod m) : ZMod 3 :=
  if x = 0 then 0
  else if x = -1 then 2
  else 1

/-- The reduction map on vertices, following Knuth's definition: given (I, J, K) in (ZMod m)³,
let i = reduce(I), j = reduce(J), s = reduce(I+J+K), and k = (s − i − j) in ZMod 3.
Note: k is NOT reduce(K); it is derived from the reduced fiber index. -/
def reduceVertex {m : ℕ} [NeZero m] (v : Vertex m) : Vertex 3 :=
  let i := reduce v.1
  let j := reduce v.2.1
  let s := reduce (fiber v)
  (i, j, s - i - j)

/-- A Hamiltonian cycle C on the m = 3 digraph is *generalizable* if the following construction
yields a Hamiltonian cycle for all odd m ≥ 3: "Given vertex (I, J, K) for general m, let
i = I', j = J', s = S', and k = (s−i−j) mod 3, where S = (I+J+K) mod m, 0' = 0, (m−1)' = 2,
and x' = 1 for 0 < x < m−1. Obtain the successor of IJK by bumping the coordinate that
cycle C bumps when forming the successor of ijk." -/
def IsGeneralizable (σ : Equiv.Perm (Vertex 3)) (_hσ : IsDirectedHamiltonianCycle (cubeDigraph 3) σ)
    : Prop :=
  ∀ m : ℕ, ∀ (_ : NeZero m), ∀ (_ : Odd m), ∀ (_ : 3 ≤ m),
    ∃ τ : Equiv.Perm (Vertex m),
      IsDirectedHamiltonianCycle (cubeDigraph m) τ ∧
      ∀ v : Vertex m, ∃ b : Fin 3,
        τ v = bumpAt b v ∧ σ (reduceVertex v) = bumpAt b (reduceVertex v)

/-- A decomposition is *Claude-like* if the direction permutation at each vertex depends only on
whether i, j, and s = (i+j+k) mod m are 0, −1, or neither. That is, the direction function
factors through the reduction map. -/
def IsClaudeLike {m : ℕ} [NeZero m]
    (d : Vertex m → Fin 3 → Fin 3) : Prop :=
  -- d is a permutation at each vertex
  (∀ v, Function.Bijective (d v)) ∧
  -- d depends only on the reduced coordinates
  (∀ v w : Vertex m, reduce v.1 = reduce w.1 → reduce v.2.1 = reduce w.2.1 →
    reduce (fiber v) = reduce (fiber w) → d v = d w)

/-! ## Counting results for m = 3

These are computational results proved by exhaustive enumeration. We state them using
existential Finset formulations to avoid decidability issues. -/

/-- The cube digraph on (ZMod 3)³ has exactly 11502 directed Hamiltonian cycles. -/
theorem hamiltonian_cycles_count_three :
    ∃ S : Finset (Equiv.Perm (Vertex 3)),
      (∀ σ ∈ S, IsDirectedHamiltonianCycle (cubeDigraph 3) σ) ∧
      (∀ σ, IsDirectedHamiltonianCycle (cubeDigraph 3) σ → σ ∈ S) ∧
      S.card = 11502 := by
  sorry

/-- There are exactly 4554 decompositions of the cube digraph on (ZMod 3)³ into three
directed Hamiltonian cycles. -/
theorem decomposition_count_three :
    ∃ S : Finset (Fin 3 → Equiv.Perm (Vertex 3)),
      (∀ f ∈ S,
        (∀ c, IsDirectedHamiltonianCycle (cubeDigraph 3) (f c)) ∧
        (∀ v : Vertex 3, ∀ b : Fin 3, ∃! c : Fin 3, bumpAt b v = f c v)) ∧
      (∀ f,
        (∀ c, IsDirectedHamiltonianCycle (cubeDigraph 3) (f c)) →
        (∀ v : Vertex 3, ∀ b : Fin 3, ∃! c : Fin 3, bumpAt b v = f c v) →
        f ∈ S) ∧
...
/-- Of the 11502 Hamiltonian cycles for m = 3, exactly 1012 generalize to
Hamiltonian cycles for m = 5. -/
theorem generalizable_to_five_count :
    ∃ S : Finset (Equiv.Perm (Vertex 3)),
      (∀ σ ∈ S,
        IsDirectedHamiltonianCycle (cubeDigraph 3) σ ∧
        ∃ τ : Equiv.Perm (Vertex 5),
          IsDirectedHamiltonianCycle (cubeDigraph 5) τ ∧
          ∀ v : Vertex 5, ∃ b : Fin 3,
            τ v = bumpAt b v ∧ σ (reduceVertex v) = bumpAt b (reduceVertex v)) ∧
      (∀ σ,
        IsDirectedHamiltonianCycle (cubeDigraph 3) σ →
...
/-- Of the 11502 Hamiltonian cycles for m = 3, exactly 996 generalize to Hamiltonian cycles
for both m = 5 and m = 7. Those 996 are in fact generalizable to all odd m > 1. -/
theorem generalizable_count :
    ∃ S : Finset (Equiv.Perm (Vertex 3)),
      (∀ σ ∈ S,
        IsDirectedHamiltonianCycle (cubeDigraph 3) σ ∧
        (∃ τ : Equiv.Perm (Vertex 5),
          ...
          ∀ v : Vertex 5, ∃ b : Fin 3,
            τ v = bumpAt b v ∧ σ (reduceVertex v) = bumpAt b (reduceVertex v)) ∧
        (∃ τ : Equiv.Perm (Vertex 7),
          IsDirectedHamiltonianCycle (cubeDigraph 7) τ ∧
          ∀ v : Vertex 7, ∃ b : Fin 3,
            τ v = bumpAt b v ∧ σ (reduceVertex v) = bumpAt b (reduceVertex v))) ∧
      (∀ σ,
        IsDirectedHamiltonianCycle (cubeDigraph 3) σ →
...
/-- The 996 cycles that generalize to both m = 5 and m = 7 are exactly those that
generalize to all odd m ≥ 3. -/
theorem generalizable_five_seven_iff_all (σ : Equiv.Perm (Vertex 3))
    (hσ : IsDirectedHamiltonianCycle (cubeDigraph 3) σ) :
    -- Generalizes to m = 5 and m = 7
    ((∃ τ : Equiv.Perm (Vertex 5),
        IsDirectedHamiltonianCycle (cubeDigraph 5) τ ∧
        ∀ v : Vertex 5, ∃ b : Fin 3,
          τ v = bumpAt b v ∧ σ (reduceVertex v) = bumpAt b (reduceVertex v)) ∧
     (∃ τ : Equiv.Perm (Vertex 7),
        ...
        ∀ v : Vertex 7, ∃ b : Fin 3,
          τ v = bumpAt b v ∧ σ (reduceVertex v) = bumpAt b (reduceVertex v)))
    ↔
    -- Generalizes to all odd m ≥ 3
    (∀ m : ℕ, ∀ (_ : NeZero m), ∀ (_ : Odd m), ∀ (_ : 3 ≤ m),
      ∃ τ : Equiv.Perm (Vertex m),
        IsDirectedHamiltonianCycle (cubeDigraph m) τ ∧
        ∀ v : Vertex m, ∃ b : Fin 3,
          τ v = bumpAt b v ∧ σ (reduceVertex v) = bumpAt b (reduceVertex v)) := by
  sorry

/-- There are exactly 760 Claude-like decompositions valid for all odd m > 1. -/
theorem claudeLike_decomposition_count :
    ∃ S : Finset (Vertex 3 → Fin 3 → Fin 3),
      (∀ d ∈ S,
        IsClaudeLike d ∧
        (∀ v : Vertex 3, ∀ b : Fin 3, ∃! c : Fin 3, d v c = b) ∧
        (∀ c : Fin 3,
          ∀ m : ℕ, ∀ (_ : NeZero m), ∀ (_ : Odd m), ∀ (_ : 1 < m),
            ∃ σ : Equiv.Perm (Vertex m), IsDirectedHamiltonianCycle (cubeDigraph m) σ ∧
              ∀ v, σ v = bumpAt (d (reduceVertex v) c) v)) ∧
      S.card = 760 := by
  sorry

/-! ## The generalizability theorem -/

/-- **Generalizability Theorem.** A Claude-like decomposition is valid for all odd m > 1 if
and only if each of the three sequences it defines for m = 3 is generalizable. -/
theorem claudeLike_valid_iff_generalizable
    (d : Vertex 3 → Fin 3 → Fin 3) (hd : IsClaudeLike d) :
    ...
    (∀ m : ℕ, ∀ (_ : NeZero m), ∀ (_ : Odd m), ∀ (_ : 1 < m),
      ∀ c : Fin 3,
        ∃ σ : Equiv.Perm (Vertex m), IsDirectedHamiltonianCycle (cubeDigraph m) σ ∧
          ∀ v, σ v = bumpAt (d (reduceVertex v) c) v)
    ↔
    -- Each of the three cycles for m = 3 is generalizable
    (∀ c : Fin 3,
      let step3 : Vertex 3 → Vertex 3 := fun v => bumpAt (d v c) v
      -- step3 is a Hamiltonian cycle for m = 3
      (∃ σ : Equiv.Perm (Vertex 3),
        IsDirectedHamiltonianCycle (cubeDigraph 3) σ ∧ ∀ v, σ v = step3 v) ∧
      -- and it generalizes to all odd m ≥ 3
      (∀ m : ℕ, ∀ (_ : NeZero m), ∀ (_ : Odd m), ∀ (_ : 3 ≤ m),
        ∃ τ : Equiv.Perm (Vertex m), IsDirectedHamiltonianCycle (cubeDigraph m) τ ∧
          ∀ v, τ v = bumpAt (d (reduceVertex v) c) v)) := by
  sorry

/-! ## Symmetry observations -/

/-- The coordinate rotation (i, j, k) ↦ (j, k, i) on vertices. -/
def rotateVertex {m : ℕ} (v : Vertex m) : Vertex m :=
  (v.2.1, v.2.2, v.1)

/-- 136 of the 760 generalizable Claude-like decompositions for m = 3 remain generalizable
under the coordinate permutation (i, j, k) ↦ (j, k, i). -/
theorem generalizable_under_rotation_count :
    ∃ S : Finset (Vertex 3 → Fin 3 → Fin 3),
      (∀ d ∈ S,
        IsClaudeLike d ∧
        -- d defines a valid decomposition for all odd m > 1
        (∀ c : Fin 3,
          ∀ m : ℕ, ∀ (_ : NeZero m), ∀ (_ : Odd m), ∀ (_ : 1 < m),
            ∃ σ : Equiv.Perm (Vertex m), IsDirectedHamiltonianCycle (cubeDigraph m) σ ∧
              ∀ v, σ v = bumpAt (d (reduceVertex v) c) v) ∧
        -- The rotated decomposition is also valid
        (∀ c : Fin 3,
          ∀ m : ℕ, ∀ (_ : NeZero m), ∀ (_ : Odd m), ∀ (_ : 1 < m),
            ∃ σ : Equiv.Perm (Vertex m), IsDirectedHamiltonianCycle (cubeDigraph m) σ ∧
              ∀ v, σ v = bumpAt (d (reduceVertex (rotateVertex v)) c) v)) ∧
      S.card = 136 := by
  sorry

/-- No Claude-like decomposition for m = 3 is invariant under all three cyclic coordinate
permutations {ijk, jki, kij}. -/
theorem no_fully_symmetric_decomposition
    (d : Vertex 3 → Fin 3 → Fin 3) (hd : IsClaudeLike d) :
    -- If d defines a valid decomposition
    (∀ c : Fin 3,
      ∀ m : ℕ, ∀ (_ : NeZero m), ∀ (_ : Odd m), ∀ (_ : 1 < m),
        ∃ σ : Equiv.Perm (Vertex m), IsDirectedHamiltonianCycle (cubeDigraph m) σ ∧
          ∀ v, σ v = bumpAt (d (reduceVertex v) c) v) →
    -- Then it is NOT invariant under both rotations
    ¬ (∀ v : Vertex 3, d (rotateVertex v) = d v) := by
  sorry
